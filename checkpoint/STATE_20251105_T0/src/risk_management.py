...
def calculate_position_size(capital: float, risk_pct: float, entry_price: float, 
                           stop_loss: float, contract_size: float = 100000) -> float:
    """
    ...
    Formula:
        ...
        Distancia_pips = |entry - stop| * 10000 (para pares forex)
        Valor_pip = contract_size / 10000 = 10 USD por pip para 1 lote standard
        Lotes = Riesgo_USD / (Distancia_pips * Valor_pip)
    """
    ...
    distance_pips = price_distance * 10000
    
    # Valor de 1 pip para 1 lote standard = 10 USD
    pip_value = 10.0
