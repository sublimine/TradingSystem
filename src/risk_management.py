...
def calculate_stop_loss_atr(*args, **kwargs):
    """
    OMEGA: deprecated range-based stop-loss helper.

    Kept only as a compatibility stub for old tests.
    This must not be used in production or for live risk decisions.
    """
    raise RuntimeError(
        "calculate_stop_loss_atr is deprecated in PLAN OMEGA and must not be used."
    )
