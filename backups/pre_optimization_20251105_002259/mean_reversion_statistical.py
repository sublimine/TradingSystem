...
class MeanReversionStatistical(StrategyBase):
    ...
    def __init__(self, config: Dict):
        ...
        self.vpin_exhaustion_threshold = config.get('vpin_exhaustion_threshold', 0.70)
        self.imbalance_reversal_threshold = config.get('imbalance_reversal_threshold', 0.40)
        self.volume_spike_multiplier = config.get('volume_spike_multiplier', 2.5)
        self.min_liquidity_score = config.get('min_liquidity_score', 0.60)
        self.reversal_velocity_min = config.get('reversal_velocity_min', 8.0)
    ...
    def _validate_microstructure_conditions(self, market_data: pd.DataFrame, features: Dict,
        ...
        validation['is_valid'] = factors_met >= 3
    ...
    def _generate_institutional_reversion_signal(self, market_data: pd.DataFrame,
        ...
        if z_score_magnitude >= 4.0 and confidence >= 0.80:
            ...
            sizing_level = 4
        elif z_score_magnitude >= 3.0 and confidence >= 0.60:
            sizing_level = 3
        else:
            sizing_level = 2
